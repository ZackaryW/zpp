...
description: Author or review Git commit messages that combine normal human-readable subjects and bodies with zmem entries or DECAY, CANCEL, and META effects. Use when an agent needs to propose, validate, or, when separately authorized, create a commit carrying durable repository memory or typed metadata changes without confusing annotations with ordinary commit prose.
---
...
5. Select the narrowest built-in type:
   ...
   - `META` when declared commit metadata must be replaced, extended, or reset over a complete ancestry range without rewriting entries.
6. Resolve every effect target from the selected trail and current Git evidence. For DECAY/CANCEL, use the target entry's unique SHA prefix and one-based annotation index. For META, use unique commit prefixes for both range endpoints and verify their ancestry; never guess an identity or infer branch applicability from a cached alias.
...
8. For DECAY, CANCEL, or META, run `zmem --repo <path> check --file <message-file> --deep` so selected history is replayed before the proposed effect. If the result reports incomplete attention or an incomplete META range, retry deliberately with `zmem --repo <path> --commit-limit -1 --node-limit -1 check --file <message-file> --deep`. Use `zmem --repo <path> check <ref> --deep` only when auditing an existing commit in its historical position.
9. Fix every validation diagnostic before presenting or committing the message. Malformed annotation-like lines still affect annotation indexing; every syntactically valid entry, custom type, DECAY, CANCEL, META, or unsupported type consumes node attention.
