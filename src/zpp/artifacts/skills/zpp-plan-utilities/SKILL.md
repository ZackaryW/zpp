...
3. Open or update one disposable companion utility OpenSpec change through the creation or revision operation owned by `openspec-propose` or `openspec-update-change`. Gate every utility to add, modify, replace, or remove before product wiring. Add it to the workflow's related change set with disposal after its complete verified TDD pass as its owning terminal condition.
...
If TDD or wiring reopens this plan, preserve coherent material tracked downstream work with a truthful fallback commit; add zmem only when the fallback is a durable temporal highlight. Otherwise return without a commit. Then suspend that stage and reconcile the complete utility change again. When revised utilities later regain complete maturity, create a replacement mature-utility checkpoint only if that pass produced material tracked utility work. Any earlier checkpoint remains historical only.
