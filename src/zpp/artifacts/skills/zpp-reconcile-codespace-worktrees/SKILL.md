...
1. Read the recorded codespace state through `zpp codespace status ID --json`.
   Require every selected generated member to retain its effective path,
   source path, branch, commit, and source/effective checkout identities.
...
4. Apply only the explicitly selected strategy, one repository at a time.
   ...
   worktrees unchanged.
5. Run the repository's relevant verification after reconciliation. Report the
   resulting commits and any branches or worktrees that remain.
