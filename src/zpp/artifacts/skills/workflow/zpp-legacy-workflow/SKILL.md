...
description: Run the complete generic current ZPP playbook for mixed or unspecialized product outcomes; this hard-cut identity provides no compatibility translation.
---

# Run the generic current workflow

This playbook owns its complete conditional sequence. It is the generic ZPP 2 outcome,
not a wrapper around the removed `zpp-workflow` or any ZPP 1 stage. Before a governed
mutation, use `zpps-workflow-kernel` for its exact guard. Read-only clarification,
exploration, verification, and finalization run directly; use the kernel afterwards
only when their result is consumed as a lifecycle gate. Its response never selects
continuation.
...
Report completion only from `lifecycle-complete`. Never invoke or interpret the
removed generic entry, former stage skills, old gate names, or compatibility behavior.
