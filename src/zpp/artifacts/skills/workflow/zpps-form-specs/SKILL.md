...
assessment carrying a durable owner, exact registered store UUID and change member,
and matching Bundler membership. If absent or mismatched, return
`kernel-assessment-required`, `durable-owner-required`, or
`store-registration-required` with the precise unresolved inputs. A repo-local root
may still be inspected and used as its trace locator, but it cannot authorize the
governed edit. A prior playbook delegation is not required.
...
`kernel-assessment-required`, `durable-owner-required`, or
`store-registration-required` when that is the blocking reason, plus changed paths
