...
description: Create the BDD bindings and compose the complete proven utility foundation into every approved feature and fix, then let the root agent verify the integrated set and prepare a justified change-wide green checkpoint.
---
...
- every planned utility green with focused tests and a current utility checkpoint when that pass produced material tracked utility work;
...
3. Do not create new utility behavior inside wiring. If any utility logic is missing, stop the entire wiring phase. Create a truthful fallback zmem checkpoint only when coherent material tracked wiring work exists; otherwise fall back without a commit. Inspect the current source graph with available code-intelligence tools such as CodeGraph, and create a fresh disposable utility OpenSpec gate for the reopened TDD pass. Resume wiring after the revised utility foundation is complete, any justified replacement checkpoint exists, and that temporary gate is discarded.
