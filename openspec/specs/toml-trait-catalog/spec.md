...
#### Scenario: Use the highest-precedence selection policy
- **WHEN** repository and global documents contribute to one family with different `selection` values
- **THEN** the repository document's selection policy governs the complete layered family
