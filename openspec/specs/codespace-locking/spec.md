...
current directory contained only by a read-only member SHALL NOT implicitly
select a codespace; the user SHALL supply its identity or activate its
environment. OpenSpec worksets SHALL NOT establish, transfer, or prove write
