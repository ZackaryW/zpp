...
Adopt user-owned `.code-workspace` files as openspec worksets and carry the governance semantics openspec refuses to hold: store bindings, member roles, and drift detection. Layout truth stays in the workspace file; semantics truth stays in the machine-local sidecar.
