...
## Purpose
Provide every supported agent integration with the validated packaged ZPP workflow family while keeping lifecycle projection independent from OpenSpec installation and generated OpenSpec skills.
...
The built ZPP distribution SHALL declare OpenSpec Bundler as a library dependency and expose `zpp` as its tool command. OpenSpec Bundler SHALL remain without a console-script entry point and SHALL NOT require or support a separate `uv tool install` step in ZPP setup. Installing the built ZPP wheel as a `uv` tool SHALL create one ZPP tool environment, make `zpp` executable, and install Bundler only inside that environment as a dependency rather than as another installed tool.

Project distribution metadata, `zpp.__version__`, `zpp --version`, and built wheel metadata SHALL expose one identical release version. A mismatch SHALL fail release and distribution verification.
