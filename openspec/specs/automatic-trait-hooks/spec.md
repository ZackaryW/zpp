...
ZPP SHALL package one native trait-context hook for each supported Agent Router agent. Each hook SHALL invoke `zpp resolve --agent <agent> .` at the earliest native context-injection event and SHALL inject only the successful command's prompt-ready complete trait bodies. The hook SHALL NOT dispatch workflow stages or grant mutation, transition, verification, or completion authority.
...
- **THEN** its native hook resolves traits with that agent identity and contributes the successful prompt-ready body output as environment policy
