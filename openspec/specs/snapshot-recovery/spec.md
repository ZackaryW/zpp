...
A local safety net for zpp's own state — recover from tampering and wrong edits. A camera for config, not a stash: sharing and restoration across machines happen by committing the `.code-workspace`, never through snapshots.
