...
def test_selected_default_home_derives_openlease_child_without_creation(
    tmp_path: Path,
) -> None:
    ...
    assert selected.state_root == user_home / ".zpp" / "openlease"
...
def test_selected_custom_home_is_normalized_without_following_symlinks(
    ...
    assert selected.state_root == tmp_path / "zpp-home" / "openlease"
    assert not selected.path.exists()
...
def test_reset_boundary_rejects_non_directory_state_child(tmp_path: Path) -> None:
    home = tmp_path / "home"
    home.mkdir()
    (home / "openlease").write_text("not a directory")

    with pytest.raises(ValueError, match=r"openlease.*directory"):
        validate_reset_boundary(ZppHome(home))


def test_prepared_state_replaces_only_openlease_child(tmp_path: Path) -> None:
    home_path = tmp_path / "home"
    old_state = home_path / "openlease"
    old_state.mkdir(parents=True)
    (old_state / "old.json").write_text("old")
    sibling = home_path / "notes.txt"
    sibling.write_text("keep")
    prepared = PreparedOpenLeaseState.prepare(ZppHome(home_path))

    prepared.replace()

    assert old_state.is_dir()
    assert list(old_state.iterdir()) == []
