...
def test_skill_projection_roots_coalesce_shared_agents_in_selection_order(
    tmp_path: Path,
) -> None:
    home = tmp_path / "home"
    ...
    assert local[1].agents == ("pi", "codex")
    ...
    assert global_roots[0].agents == ("codex", "pi")
...
def test_inspect_skill_scopes_preserves_coalesced_projection_order(tmp_path: Path) -> None:
    ...
    assert tuple(item.state for item in inspections) == ("absent", "absent")
